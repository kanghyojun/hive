...
사이드바를 처음 열면 백그라운드 수집기가 자동으로 시작됩니다. 같은 `HIVE_HOME`(심볼릭 링크를 해소한 경로)과 tmux 서버(소켓 경로·PID·시작 시각)를 쓰는 사이드바는 수집기 하나를 공유합니다. 수집기는 이벤트와 pane 상태를 모으고 30초마다 cron을 판정합니다. TUI는 수집기가 보낸 상태를 표시하고 사용자 입력을 전달합니다. 모든 사이드바를 닫아도 수집과 cron은 계속됩니다. 같은 예정 시각의 중복 실행은 기존 `cron_runs`의 `UNIQUE(job_id, fire_at)` 제약으로 막습니다.

해당 tmux 서버가 종료되거나 교체되면 수집기도 종료합니다. 수집기가 비정상 종료되면 연결된 사이드바가 다시 연결하고 수집기를 자동으로 시작합니다. 연결이 끊기거나 마지막 수집이 지연되면 상태바에 `갱신 대기` 또는 `갱신 중단`을 표시합니다. **사이드바가 하나도 없는 동안 수집기까지 죽으면 다음 사이드바 실행 전까지 자동으로 재시작하지 않습니다.** 수집기 시작 실패와 종료 오류는 `HIVE_HOME/logs/collector.log`에 남습니다.
...
잡 하나에 오타가 나도 나머지는 그대로 돕니다. 걸러낸 이유는 `hive cron list`에 `!`로 붙고, 자동 cron 실행 실패는 `~/.hive/logs/cron.log`에 남습니다.
